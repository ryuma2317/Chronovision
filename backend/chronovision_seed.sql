...
SELECT UUID(), 1, 'What number comes next in the sequence: 2, 4, 6, 8, ?'
WHERE NOT EXISTS (SELECT 1 FROM aptitude_questions WHERE question_order = 1);

INSERT INTO aptitude_questions (question_id, question_order, question_text)
SELECT UUID(), 2, 'Book is to Reading as Fork is to ____.'
WHERE NOT EXISTS (SELECT 1 FROM aptitude_questions WHERE question_order = 2);

INSERT INTO aptitude_questions (question_id, question_order, question_text)
SELECT UUID(), 3, 'Which word does NOT belong with the others: Apple, Banana, Carrot, Mango?'
WHERE NOT EXISTS (SELECT 1 FROM aptitude_questions WHERE question_order = 3);

INSERT INTO aptitude_questions (question_id, question_order, question_text)
SELECT UUID(), 4, 'If all Zips are Zaps, and all Zaps are Zops, then all Zips are definitely ____.'
WHERE NOT EXISTS (SELECT 1 FROM aptitude_questions WHERE question_order = 4);

INSERT INTO aptitude_questions (question_id, question_order, question_text)
SELECT UUID(), 5, 'What number comes next: 1, 2, 4, 8, 16, ?'
WHERE NOT EXISTS (SELECT 1 FROM aptitude_questions WHERE question_order = 5);

INSERT INTO aptitude_questions (question_id, question_order, question_text)
SELECT UUID(), 6, 'A clock shows 3:15. What is the angle relationship between the hour and minute hands closest to?'
WHERE NOT EXISTS (SELECT 1 FROM aptitude_questions WHERE question_order = 6);

INSERT INTO aptitude_questions (question_id, question_order, question_text)
SELECT UUID(), 7, 'Hot is to Cold as Up is to ____.'
WHERE NOT EXISTS (SELECT 1 FROM aptitude_questions WHERE question_order = 7);

INSERT INTO aptitude_questions (question_id, question_order, question_text)
SELECT UUID(), 8, 'Complete the pattern: A, C, E, G, ?'
WHERE NOT EXISTS (SELECT 1 FROM aptitude_questions WHERE question_order = 8);

INSERT INTO aptitude_questions (question_id, question_order, question_text)
SELECT UUID(), 9, 'If 5 machines make 5 toys in 5 minutes, how many minutes would it take 100 machines to make 100 toys?'
WHERE NOT EXISTS (SELECT 1 FROM aptitude_questions WHERE question_order = 9);

INSERT INTO aptitude_questions (question_id, question_order, question_text)
SELECT UUID(), 10, 'Which shape would logically complete a set where every figure has one more side than the last, starting at a triangle (3 sides), then 4, then 5 — what comes next?'
WHERE NOT EXISTS (SELECT 1 FROM aptitude_questions WHERE question_order = 10);

INSERT INTO aptitude_questions (question_id, question_order, question_text)
SELECT UUID(), 11, 'Doctor is to Hospital as Teacher is to ____.'
WHERE NOT EXISTS (SELECT 1 FROM aptitude_questions WHERE question_order = 11);

INSERT INTO aptitude_questions (question_id, question_order, question_text)
SELECT UUID(), 12, 'What number comes next: 3, 6, 12, 24, ?'
WHERE NOT EXISTS (SELECT 1 FROM aptitude_questions WHERE question_order = 12);

INSERT INTO aptitude_questions (question_id, question_order, question_text)
SELECT UUID(), 13, 'A train travels 60 km in 1 hour. At the same speed, how far does it travel in 3 hours?'
WHERE NOT EXISTS (SELECT 1 FROM aptitude_questions WHERE question_order = 13);

INSERT INTO aptitude_questions (question_id, question_order, question_text)
SELECT UUID(), 14, 'Which word does NOT belong with the others: Triangle, Square, Circle, Rectangle?'
WHERE NOT EXISTS (SELECT 1 FROM aptitude_questions WHERE question_order = 14);

INSERT INTO aptitude_questions (question_id, question_order, question_text)
SELECT UUID(), 15, 'If today is Wednesday, what day will it be 100 days from now?'
